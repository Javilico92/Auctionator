...
function Atr_FullScanFrameIdle()
	...
	--QueryAuctionItems ("", nil, nil, 0, 0, 0, gSlowScanPage, 0, nil)
		...
		if (gSlowScanTotalPages) then
			statusText = string.format (ZT("Page %s of %s"), gSlowScanPage+1, gSlowScanTotalPages)
		end
	end
  ...
  if (gAtr_FullScanState == ATR_FS_ANALYZING )  then  -- statusText = ZT("Analyzing data").." ["..gFullScanPosition.." "..ZT("out of").." "..gGetAllTotalAuctions.."]";        end
		statusText = ZT("Analyzing data").." ["..gFullScanPosition.." "..ZT("out of").." "..gGetAllTotalAuctions.."]"
